...
/*   Updated: 2025/01/23 19:32:30 by jealefev         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	philosopher_dead(t_philo *philo)
{
	...
	if (get_time() - philo->last_meal >= philo->global_state->time_to_die)
	{
		...
		printf("%ld %d died--\n", get_time() - philo->global_state->start_time, philo->id);
...
void	*routine(void *data)
{
	...
	if (philo->id % 2)
		ft_usleep(1, philo->global_state);
	while (philosopher_dead(philo) == 0
		&& safe_check_death(philo->global_state) == false)
	{
		if (safe_check_death(philo->global_state) == true || philosopher_dead(philo) || _eat(philo) == 1)
			return (NULL) ;
		if (safe_check_death(philo->global_state) == true || philosopher_dead(philo) || _sleep(philo) == 1)
			return (NULL) ;
		if (safe_check_death(philo->global_state) == true || philosopher_dead(philo) || _think(philo) == 1)
				return (NULL) ;
		// if (philo->id != 1)
		// 	if (safe_check_death(philo->global_state) == true || philosopher_dead(philo) || _think(philo) == 1)
		// 		return (NULL) ;
